...
def main(cfg : DictConfig) -> None:
    ...
    # motion_file = f"data/{cfg.robot.humanoid_type}/{cfg.motion_name}.pkl"
    motion_file = cfg.motion_name
    print(motion_file)
    motion_data = joblib.load(motion_file)
    data_list = motion_data
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data, key_callback=key_call_back) as viewer:
        ...
        for i in range(motion_data_list.shape[0]):
            curr_motion = motion_data_list[i]
            for curr_time in range(curr_motion['dof'].shape[0]):
                ...
                for i in range(24):
                    viewer.user_scn.geoms[i+1].pos = curr_motion['smpl_joints'][curr_time, i]
